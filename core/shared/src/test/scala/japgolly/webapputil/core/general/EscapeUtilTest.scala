  ...
  override def tests = Tests {
    ...
    "quoteAndEscape" - {
      def test(input: String, expected: String = null)(implicit l: Line): Unit = {
        ...
        val expectQ = "\"" + expect + "\""
        assertEq("escape", EscapeUtil.escape(input), expect)
        assertEq("quote", EscapeUtil.quote(input), expectQ)
        assertEq("escape sb", withSB(EscapeUtil.appendEscaped(_, input)), expect)
        assertEq("quote sb", withSB(EscapeUtil.appendQuoted(_, input)), expectQ)
      }
      "noNeed" - {
        "empty" - test("")
        "asciiMid" - {
