...
object AjaxExampleJs {
  ...
  //   - out-of-protocol errors (eg. client lost connectivity) are caught and converted into a Left[ErrorMsg]
  ...
  object ExampleComponent {
    ...
    //
    // The component doesn't care about the details of how call is made, which makes it super easy to test.
    // In fact, it needn't even be an AJAX call! It could be a websocket call, a webworker call, etc. Whoever uses the
    // component gets to decide, no changes here are necessary.
    //
    final case class Props(addIntsAjax: ServerSideProcInvoker[Request, ErrorMsg, Response])
    ...
    final class Backend($: BackendScope[Props, State]) {
      ...
      private val onSubmit =
        for {
          ...
          result <- p.addIntsAjax(req) // no need to catch errors here, as per the ServerSideProcInvoker contract
          _      <- $.setStateAsync(State.Finished(result))
        } yield ()

      private def submitButton =
        <.button(
          s"What really is ${req.m} + ${req.n}? Ask our proprietary Addition Service for free!",
          ^.onClick --> onSubmit)
      ...
      def render(s: State): VdomNode =
        s match {
          ...
          case State.Finished(Right(res)) => <.div(s"${req.m} + ${req.n} = $res! Amazing!!")
  ...
  // ===================================================================================================================
  // Here is another example of making AJAX calls from a React component, this time in a way that makes lower-level
  ...
  object ExampleComponent2 {
    ...
    // The JsonAjaxClient could be a real one that makes real calls, or an in-memory instance for lower-level testing.
    ...
    final class Backend($: BackendScope[Props, State]) {
      ...
      private val onSubmit =
        for {
          _      <- $.setStateAsync(State.Pending)
          p      <- $.props.asAsyncCallback
          invoker = p.ajaxClient.invoker(protocol) // create our own invoker using the provided AjaxClient
          result <- invoker(req) // again: no need to catch errors here, as per the ServerSideProcInvoker contract
          _      <- $.setStateAsync(State.Finished(result))
        } yield ()
